...
def getFGSM(batch_size, DATASET, NET_TYPE):
    outf = os.path.join('adv_output','{}_{}/'.format(NET_TYPE, DATASET))
    test_clean_data = torch.load(outf + 'clean_data_{}_{}_FGSM.pth'.format(NET_TYPE, DATASET))
    test_adv_data = torch.load(outf + 'adv_data_{}_{}_FGSM.pth'.format(NET_TYPE, DATASET))
    test_noisy_data = torch.load(outf + 'noisy_data_{}_{}_FGSM.pth'.format(NET_TYPE, DATASET))
    test_label = torch.load(outf + 'label_{}_{}_FGSM.pth'.format(NET_TYPE, DATASET))

    test_in = test_clean_data
    test_out = test_adv_data
    test_noisy = test_noisy_data
    test_pos = torch.cat((test_in, test_noisy), 0)
    test_loader = torch.utils.data.DataLoader(test_pos, batch_size=batch_size, shuffle=False, num_workers=1)
    out_loader = torch.utils.data.DataLoader(test_out, batch_size=batch_size, shuffle=False, num_workers=1)

    return test_loader, out_loader
